...
st.title("Most popular repo demo - Bedrock Agents version")
...
def execute_state_machine():
    execution_arn = stepfn.start_execution(
        "PromptChainDemo-MostPopularRepoBedrockAgents",
        st.session_state.user_id,
        "{}",
    )
    st.session_state.most_popular_repo_ba_execution_arn = execution_arn
...
with behind_the_scenes_col:
    ...
    if "most_popular_repo_ba_execution_arn" in st.session_state:
        status_markdown = stepfn.describe_execution(
            st.session_state.most_popular_repo_ba_execution_arn
        )
...
with demo_col:
    ...
    with st.form("start_most_popular_repo_ba_demo_form"):
        st.info(
        ...
        if started:
            with st.spinner("Wait for it..."):
                if "most_popular_repo_ba_execution_arn" in st.session_state:
                    del st.session_state["most_popular_repo_ba_execution_arn"]
                display_no_state_machine_status()
                response = execute_state_machine()

                st.session_state.most_popular_repo_ba_execution_status = response[
                    "status"
                ]
                if response["status"] == "SUCCEEDED":
                    output = json.loads(response["output"])
                    st.session_state.most_popular_repo_ba_url = output["repo"]
                    st.session_state.most_popular_repo_ba_summary = output["summary"]

            if st.session_state.most_popular_repo_ba_execution_status == "SUCCEEDED":
                st.success("Done!")
                st.write(
                    "The most popular repository on GitHub today is: ",
                    st.session_state.most_popular_repo_ba_url,
                )
                st.write(st.session_state.most_popular_repo_ba_summary)
            else:
                st.error(
